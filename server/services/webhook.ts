...
import type { ParsedReport } from "../parser/junit.js";
import type { TrendResult } from "../parser/store.js";
...
export function buildFallbackAlert(report: ParsedReport, trend?: TrendResult): string {
  const topFailure = report.failures.find((f: any) => f.highImpact) || report.failures[0];
  const lines = [
 ...
 */
export async function notifyWebhook(text: string, webhookUrlOverride?: string): Promise<WebhookNotifyResult> {
  ...
  try {
    ...
    const ok = res.ok;
    if (!ok) logger.warn("Webhook notification rejected", { status: res.status });
