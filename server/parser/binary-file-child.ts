  ...
  const sources = workbook.SheetNames.map((name) => {
    ...
    if (worksheet["!ref"]) {
      ...
      if (range.e.r - range.s.r + 1 > MAX_ROWS_PER_SHEET) {
        throw new Error(
          `The worksheet "${name}" exceeds the ${MAX_ROWS_PER_SHEET} row limit.`
        );
      }
      if (range.e.c - range.s.c + 1 > MAX_COLUMNS) {
        throw new Error(
          `The worksheet "${name}" exceeds the ${MAX_COLUMNS} column limit.`
        );
      }
    }
    ...
    for (const row of rows) {
      ...
      if (cellCount > MAX_CELLS) {
        throw new Error(
          `The workbook exceeds the ${MAX_CELLS.toLocaleString()} cell limit.`
        );
      }
    }
...
async function parsePdf(bytes: Uint8Array) {
  ...
  try {
    ...
    let sources: Array<{ name: string; rows: string[][] }> = [];
    try {
      const tableResult = await parser.getTable();
      sources = tableResult.pages.flatMap((page) =>
...
async function main(input: ChildInput) {
  try {
    const result =
      input.kind === "pdf"
...
process.once("message", (message) => {
  void main(message as ChildInput);
});
