...
import type { TabularSource } from "./xml.js";
...
type SupportedExtension = (typeof SUPPORTED_REPORT_EXTENSIONS)[number];

interface BinaryWorkerResult {
  ok: boolean;
  error?: string;
  text?: string;
...
function reportExtension(fileName: string): SupportedExtension {
  const extension = path.extname(fileName).toLowerCase();
  if (
    !SUPPORTED_REPORT_EXTENSIONS.includes(extension as SupportedExtension)
  ) {
    throw new Error(
      `Unsupported report type "${extension || "(none)"}". Expected PDF, Excel, HTML, XML, JSON, CSV, or TXT.`
    );
  }
...
function runBinaryParser(
  ...
  return new Promise((resolve, reject) => {
    const child = fork(
      new URL("./binary-file-child.js", import.meta.url),
      [],
      {
        execArgv: [
    ...
    const timer = setTimeout(() => {
      finish(() => {
        ...
        reject(
          new Error(
            `${kind === "pdf" ? "PDF" : "Excel"} parsing exceeded the ${BINARY_TIMEOUT_MS / 1000}-second limit.`
          )
        );
      });
    }, BINARY_TIMEOUT_MS);

    child.once("message", (message: BinaryWorkerResult) => {
      finish(() => {
        if (message.ok) resolve(message);
        else reject(new Error(message.error || "Binary report parsing failed."));
      });
    });
    ...
    child.once("exit", (code, signal) => {
      finish(() =>
        reject(
          new Error(
            `Binary report parser stopped unexpectedly (${signal || code || "unknown"}).`
          )
        )
      );
    });
...
function parseTextFile(extension: SupportedExtension, text: string): ParsedReport {
  switch (extension) {
    case ".html":
...
async function parsePdf(bytes: Uint8Array): Promise<ParsedReport> {
  ...
  if (!text) {
    throw new Error(
      "The PDF contains no extractable text. Scanned/image-only PDFs require OCR and are not supported."
    );
  }
  return parseAnyReport(text);
}

async function parseSpreadsheet(bytes: Uint8Array): Promise<ParsedReport> {
...
export async function parseReportBytes(
  ...
  if (bytes.byteLength === 0) throw new Error("The report file is empty.");
...
export async function parseBase64ReportFile(
  fileName: string,
...
export function canonicalReportText(report: ParsedReport): string {
