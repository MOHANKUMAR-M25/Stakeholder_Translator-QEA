...
import { ingestReport, getReport, computeTrend } from "../parser/store.js";
import { notifyWebhook, buildFallbackAlert } from "./webhook.js";
import { logger } from "../utils/logger.js";
const AUDIENCE_SPEC = {
    dm: {
        label: "Delivery Manager",
        instructions: "Write a 3-line, SMS-style update. Tight, operational, no fluff. What ran, what broke, " +
            "what needs a decision from them today. No preamble, no sign-off — just the 3 lines.",
    },
    po: {
        label: "Product Owner",
        instructions: "Write a 1-page narrative for someone who owns the roadmap, not the test suite. Cover: " +
    ...
    client: {
        label: "Client Slide",
        instructions: "Write a risk-flagged slide outline for an external client. Start with a RAG flag on its " +
...
const SHARED_PREAMBLE = "You are the Stakeholder Translator, an AI agent built for QA leads. You take raw, technical " +
...
function formatTrendForPrompt(trend) {
    if (!trend.hasPrevious)
        return "  (no previous run recorded yet — this is the first one)";
    const lines = [`  - ${trend.summary}`];
    if (trend.newlyFailing.length)
        lines.push(`  - Newly failing since last run: ${trend.newlyFailing.join(", ")}`);
    if (trend.newlyFixed.length)
        lines.push(`  - Newly fixed since last run: ${trend.newlyFixed.join(", ")}`);
    return lines.join("\n");
}
function buildStandardPrompt(audience, reportId, trend) {
    const spec = AUDIENCE_SPEC[audience];
    const report = getReport(reportId);
    const suiteLines = report.suites
        .map((s) => `  - ${s.name}: ${s.passed}/${s.total} passed, ${s.failed} failed, ${s.skipped} skipped`)
        .join("\n");
    const failureLines = report.failures
        .slice(0, 20)
        .map((f) => `  - [${f.highImpact ? "HIGH IMPACT" : "low impact"}] ${f.suite} · ${f.test}: ${f.message}`)
        .join("\n");
...
function buildAgenticPrompt(audience, reportId) {
    const spec = AUDIENCE_SPEC[audience];
    return `${SHARED_PREAMBLE}
