import { NORMALIZED_REPORT_FORMAT, asNonNegativeInteger, asNonNegativeNumber, boundedText, finalizeReport, isHighImpact, } from "./model.js";
...
function normalizeStatus(value) {
    ...
    if (status.includes("skip") || status.includes("pending"))
        return "skipped";
...
function durationSeconds(value, header = "") {
    ...
    if (secondPart)
        seconds += Number(secondPart[1]);
    if (milliseconds)
        seconds += Number(milliseconds[1]) / 1000;
    if (seconds > 0)
        return seconds;
...
function tableLimits(sources) {
    let cells = 0;
    for (const source of sources) {
        if (source.rows.length > MAX_TABULAR_ROWS + 20) {
            throw new Error(`The table "${source.name}" exceeds the ${MAX_TABULAR_ROWS.toLocaleString()} row limit.`);
        }
        for (const row of source.rows) {
            ...
            if (row.length > MAX_TABULAR_COLUMNS) {
                throw new Error(`The table "${source.name}" exceeds the ${MAX_TABULAR_COLUMNS} column limit.`);
            }
            cells += row.length;
            if (cells > MAX_TABULAR_CELLS) {
                throw new Error(`The report exceeds the ${MAX_TABULAR_CELLS.toLocaleString()} cell limit.`);
            }
        }
    }
}
function parseDetailedTable(source) {
    ...
    for (let headerIndex = 0; headerIndex < headerLimit; headerIndex += 1) {
        ...
        for (const row of source.rows.slice(headerIndex + 1)) {
            ...
            if (!status)
                continue;
            const suiteName = boundedText(cell(row, suiteIndex), source.name || "Test report", 160);
            const testName = boundedText(cell(row, testIndex), `Test ${recognizedRows + 1}`, 200);
            ...
            recognizedRows += 1;
            const rowDuration = durationSeconds(cell(row, durationIndex), cell(headers, durationIndex));
            durationSec += rowDuration;
            if (status === "failed") {
                const message = boundedText(cell(row, messageIndex), "Failed", 300);
                failures.push({
                    suite: suiteName,
                    test: testName,
                    message,
...
function parseAggregateTable(source) {
    const headerLimit = Math.min(source.rows.length, 15);
    for (let headerIndex = 0; headerIndex < headerLimit; headerIndex += 1) {
        const headers = source.rows[headerIndex];
        ...
        for (const row of source.rows.slice(headerIndex + 1)) {
            ...
            const explicitPassed = asNonNegativeInteger(numericCell(cell(row, passedIndex)));
            const passed = passedIndex >= 0
                ? explicitPassed
            ...
            candidates.push({
                ...
                failed,
                skipped,
                durationSec: durationSeconds(cell(row, durationIndex), cell(headers, durationIndex)),
            });
        }
        if (candidates.length === 0)
            continue;
        const filtered = candidates.length > 1
            ? candidates.filter((candidate) => !/^(grand\s*total|overall|all|summary|total)$/i.test(candidate.name))
            : candidates;
        ...
        return {
            ...
            failures: [],
            durationSec: selected.reduce((sum, candidate) => sum + candidate.durationSec, 0),
        };
    }
    return null;
}
export function parseTabularSources(sources) {
    tableLimits(sources);
    const detailed = [];
    const aggregate = [];
    for (const source of sources) {
    ...
    if (detailed.length > 0 && aggregate.length > 0) {
        const aggregateDuration = aggregate.reduce((sum, result) => sum + result.durationSec, 0);
        return finalizeReport(aggregate.flatMap((result) => result.suites), detailed.flatMap((result) => result.failures), aggregateDuration ||
            detailed.reduce((sum, result) => sum + result.durationSec, 0));
    }
    const selected = detailed.length > 0 ? detailed : aggregate;
    if (selected.length === 0) {
        throw new Error("No recognizable test table was found. Include test/name and status/result columns, or suite totals with passed/failed/skipped counts.");
    }
    return finalizeReport(selected.flatMap((result) => result.suites), selected.flatMap((result) => result.failures), selected.reduce((sum, result) => sum + result.durationSec, 0));
}
...
export function parseDelimitedRows(text) {
    ...
    if (quoted)
        throw new Error("Malformed delimited report: an opening quote is not closed.");
...
function recordName(record, fallback) {
    return boundedText(firstDefined(record, [
        "name",
        "title",
        "test",
        "testName",
...
function recordDuration(record, commonUnit = "seconds") {
    ...
    if (seconds !== undefined)
        return asNonNegativeNumber(seconds);
    ...
    if (milliseconds !== undefined)
        return asNonNegativeNumber(milliseconds) / 1000;
...
function collectTestRecords(value, suiteName, output, durationUnit = "seconds", inheritedTestName = "") {
    if (Array.isArray(value)) {
        for (const entry of value) {
            collectTestRecords(entry, suiteName, output, durationUnit, inheritedTestName);
        }
        return;
    }
    ...
    const contextualTestName = ownTestName !== undefined
    ...
    for (const key of [
        ...
        if (Array.isArray(record[key])) {
            collectTestRecords(record[key], nestedSuite, output, key === "assertionResults" ? "milliseconds" : durationUnit, key === "tests" && contextualTestName
                ? contextualTestName
                : inheritedTestName);
        }
    }
    if (output.length > beforeNested)
        return;
    const retryResults = Array.isArray(record.results)
        ? record.results.map(asRecord).filter((entry) => Boolean(entry))
        : [];
    if (retryResults.length > 0 &&
        retryResults.every((entry) => Boolean(normalizeStatus(firstDefined(entry, ["status", "result", "outcome", "state"])))) &&
        retryResults.every((entry) => firstDefined(entry, ["name", "title", "test", "testName"]) === undefined) &&
        Boolean(contextualTestName)) {
        const finalAttempt = retryResults[retryResults.length - 1];
        const attemptStatus = normalizeStatus(firstDefined(finalAttempt, ["status", "result", "outcome", "state"]));
        const overallStatusText = String(record.status ?? "").toLowerCase();
        const useOverallStatus = record.expectedStatus !== undefined ||
            record.projectName !== undefined ||
            ["expected", "unexpected", "flaky"].includes(overallStatusText);
        const finalStatus = (useOverallStatus ? normalizeStatus(record.status) : null) ?? attemptStatus;
        if (finalStatus) {
            output.push({
    ...
    if (Array.isArray(record.results)) {
        collectTestRecords(record.results, nestedSuite, output, durationUnit, contextualTestName || inheritedTestName);
    }
}
function recordsToResult(records) {
    const suites = new Map();
    const failures = [];
    let durationSec = 0;
    for (const record of records) {
        const suite = suites.get(record.suite) ??
...
function aggregateSuite(value, fallbackName) {
    ...
    const passed = passedValue !== undefined
        ? providedPassed
        : Math.max(providedTotal - failed - skipped, 0);
    const total = Math.max(providedTotal, passed + failed + skipped);
    return {
        name: boundedText(firstDefined(record, ["name", "title", "suite", "suiteName", "module"]), fallbackName, 160),
        total,
        passed,
        failed,
        skipped,
...
function topLevelFailures(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map(asRecord)
        ...
        return {
            ...
            test,
            message,
            highImpact: Boolean(failure.highImpact) ||
...
function parseJsonValue(data) {
    ...
    const container = root && asRecord(root.report) && !root.suites && !root.testsuites
        ? root.report
        : root;
    if (container?.format === NORMALIZED_REPORT_FORMAT &&
        ...
        return finalizeReport(suites, topLevelFailures(container.failures), asNonNegativeNumber(container.durationSec));
    }
    ...
    if (rawSuites) {
        ...
        rawSuites.forEach((rawSuite, index) => {
            ...
            if (!suiteRecord)
                return;
            const suiteName = boundedText(firstDefined(suiteRecord, [
                "name",
                "title",
                "suite",
                "suiteName",
            ...
            if (records.length > 0) {
                ...
                if (aggregate) {
                    ...
                    durationSec += asNonNegativeNumber(firstDefined(suiteRecord, ["duration", "time", "durationSec"]));
                }
            }
        });
        if (suites.length > 0) {
            const explicitFailures = topLevelFailures(container?.failures);
            return finalizeReport(suites, failures.length > 0 ? failures : explicitFailures, durationSec);
        }
    }
    if (container) {
        const records = [];
        ...
        const aggregate = aggregateSuite(container, "Test report") ??
            aggregateSuite(stats, "Test report");
        if (aggregate) {
            const statsDuration = stats && stats.duration !== undefined
                ? asNonNegativeNumber(stats.duration) / 1000
                : 0;
            return finalizeReport([aggregate], topLevelFailures(container.failures), statsDuration ||
                asNonNegativeNumber(firstDefined(container, ["durationSec", "duration", "time"])));
        }
    }
...
function statusFromLabel(label) {
    ...
    if (/^total(?: tests?)?$/.test(normalized))
        return "total";
    if (/^(error|errors)$/.test(normalized))
        return "error";
...
function summaryCandidate(line) {
    ...
    const seen = new Set();
    for (const match of line.matchAll(/(\d+)\s+(passed|passing|failed|failing|failures?|errors?|skipped|pending|ignored|disabled|xfailed|xpassed|total(?:\s+tests?)?)/gi)) {
    ...
    for (const match of line.matchAll(/(passed|passing|failed|failing|failures?|errors?|skipped|pending|ignored|disabled|xfailed|xpassed|total(?:\s+tests?)?)\s*[:=]\s*(\d+)/gi)) {
...
function parseStatusLines(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (!trimmed)
            continue;
        const leading = trimmed.match(/^(?:\[\s*)?(PASS(?:ED)?|FAIL(?:ED)?|ERROR|SKIP(?:PED)?|PENDING)(?:\s*\])?[\s:|\-]+(.+)$/i);
        const trailing = trimmed.match(/^(.+?)[\s|:,-]+(PASS(?:ED)?|FAIL(?:ED)?|ERROR|SKIP(?:PED)?|PENDING)$/i);
...
export function parseLogReport(text) {
    ...
    if (surefire) {
        ...
        if (!candidate) {
            ...
            if (lineReport)
                return lineReport;
            throw new Error("Could not recognize test results in this text. Include a pass/fail summary or rows with test names and statuses.");
        }
    ...
    const failNames = [
        ...text.matchAll(/^(?:FAILED|FAIL|ERROR|\[\s*FAIL(?:ED)?\s*\])[\s:|\-]+(.+)$/gim),
    ...
    const durationMatch = text.match(/\b(?:duration|time|elapsed)\s*[:=]\s*([0-9:.]+\s*(?:ms|s|sec|m|min|h)?)/i) ??
        text.match(/\bin\s+([0-9.]+\s*(?:ms|s|sec|m|min|h))\b/i);
    const durationSec = durationMatch ? durationSeconds(durationMatch[1]) : 0;
    return finalizeReport([{ name: "Test report", total, passed, failed, skipped }], failures, durationSec);
}
