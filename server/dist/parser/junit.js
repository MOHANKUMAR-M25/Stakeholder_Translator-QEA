...
export function parseJUnitXML(xml) {
    ...
    const suiteNodes = asArray(root.testsuite);
    ...
    for (const s of suiteNodes) {
        ...
        for (const tc of asArray(s.testcase)) {
            const failNode = tc.failure ?? tc.error;
            if (!failNode)
                continue;
            const testName = tc["@_name"] || "unnamed test";
            const message = (typeof failNode === "object" ? failNode["@_message"] || failNode["#text"] : String(failNode)) || "Failed";
            failures.push({
                suite: name,
                test: testName,
