...
import { asNonNegativeInteger, asNonNegativeNumber, boundedText, finalizeReport, isHighImpact, } from "./model.js";
function asArray(value) {
    if (value === undefined)
        return [];
    return Array.isArray(value) ? value : [value];
}
function directTestCases(suite) {
    return asArray(suite.testcase);
}
function collectLeafSuites(node, output) {
    if (!node || typeof node !== "object")
        return;
    const record = node;
    const children = asArray(record.testsuite);
    if (children.length > 0) {
        for (const child of children)
            collectLeafSuites(child, output);
        if (directTestCases(record).length > 0)
            output.push(record);
        return;
    }
    if ("testcase" in record ||
        "@_tests" in record ||
        "@_failures" in record ||
        "@_errors" in record) {
        output.push(record);
    ...
    for (const [key, value] of Object.entries(record)) {
        if (key.startsWith("@_") || key === "testcase")
            continue;
        collectLeafSuites(value, output);
    }
}
function nestedMessage(node) {
    if (!node || typeof node !== "object")
        return boundedText(node, "Failed");
    const record = node;
    return boundedText(record["@_message"] ??
        record.message ??
        record["full-stacktrace"] ??
        record["#text"] ??
        record, "Failed");
}
function parseJUnitDocument(doc) {
    const suiteNodes = [];
    ...
    for (const suiteNode of suiteNodes) {
        ...
        const declaredFailed = asNonNegativeInteger(suiteNode["@_failures"]) +
            asNonNegativeInteger(suiteNode["@_errors"]);
        const failed = Math.max(declaredFailed, observedFailed);
        const skipped = Math.max(asNonNegativeInteger(suiteNode["@_skipped"]), observedSkipped);
        const declaredTotal = asNonNegativeInteger(suiteNode["@_tests"]);
        const total = Math.max(declaredTotal, testCases.length, observedPassed + failed + skipped);
...
function visitTestNg(node, state) {
    if (Array.isArray(node)) {
        for (const entry of node)
            visitTestNg(entry, state);
        return;
    }
    if (!node || typeof node !== "object")
        return;
    const record = node;
    for (const suite of asArray(record.suite)) {
    ...
    for (const method of asArray(record["test-method"])) {
        if (String(method["@_is-config"] ?? "").toLowerCase() === "true")
            continue;
    ...
    for (const [key, value] of Object.entries(record)) {
        if (key === "suite" || key === "test-method" || key.startsWith("@_"))
            continue;
        visitTestNg(value, state);
    }
}
function parseTestNgDocument(doc) {
    if (!("testng-results" in doc))
        return null;
    const state = {
        suiteName: "TestNG",
        suites: new Map(),
...
export function parseJUnitXML(xml) {
    ...
    if (junit)
        return junit;
    throw new Error("No JUnit <testsuite> or TestNG <test-method> results were found.");
}
