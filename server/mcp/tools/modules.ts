...
export const getModuleTool = {
  ...
  handler: async ({ reportId, name }: { reportId: string; name: string }) => {
    const r = getReport(reportId);
    const suite = r.suites.find((s: any) => s.name.toLowerCase() === name.toLowerCase());
    if (!suite) return { found: false, availableNames: r.suites.map((s: any) => s.name) };
    const failures = r.failures.filter((f: any) => f.suite.toLowerCase() === name.toLowerCase());
