...
app.post("/api/notify-test", async (req, res) => {
  const body = req.body as { reportId?: string; webhookUrl?: string; message?: string };
  try {
...
function validateReportPayload(body: ReportPayloadBody): string | null {
...
app.post("/api/parse", async (req, res) => {
  const body = req.body as ReportPayloadBody;
...
app.post("/api/translate", async (req, res) => {
  ...
  if (reportValidationError) return res.status(400).json({ error: reportValidationError });
